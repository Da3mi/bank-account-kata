...
public class BankCustomerOperationTest {
	...
	public void customer_should_check_operations() {
		...
		String expectedAccountHistoryheader = String.format(ACCOUNT_HEADER_TEMPLATE , expectedCustomer.getName(), LocalDate.now(), LocalDate.now().atStartOfDay(), expectedCustomer.getAccount().getAccountBalance());
		...
		String operation2 = String.format(ACCOUNT_OPERATION_LINE_TEMPLATE , withdrawTransaction1.getTransactionDate(), 
				...
				expectedCustomer.getAccount().getAccountBalance().add(depositValue).subtract(withdrawValue).subtract(withdrawValue));
